...
function makeHtml(result, showToneColors) {
    ...
    for (let i = 0; i < result.data.length; ++i) {
        ...
        if (config.simpTrad === 'auto') {
            ...
            const [firstHanzi, secondHanzi] = config.simpTradOrder === 'orderSimpFirst' ?
                  [entry[2], entry[1]] :  [entry[1], entry[2]]
            html += '<span class="' + hanziClass + '">' + firstHanzi + '</span>&nbsp;';
            if (entry[1] !== entry[2]) {
                html += '<span class="' + hanziClass + '">' + secondHanzi + '</span>&nbsp;';
            }
        ...
        let translation = entry[4].replace(/([^<])\//g, '$1; ');  // will not replace the slash when it starts the string
